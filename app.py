...
class NavSatEfficiencyModel:
    """Wrapper around a RandomForestRegressor model.

    The model is trained on synthetic data that encodes a
    ...
    RandomForestRegressor is used instead of LinearRegression because the
    efficiency surface is Gaussian (non-linear), which a flat linear model
    cannot capture — causing it to return nearly identical predictions for
    all inputs.
    """
